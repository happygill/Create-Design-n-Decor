package uwu.bbb.design_decor.infrastructure.config_menu;
...
public class OpenDecorMenuButton extends Button {

    public static final ItemStack ICON = AllItems.GOGGLES.asStack();

    public OpenDecorMenuButton(int x, int y) {
        super(x, y, 20, 20, Components.immutableEmpty(), OpenDecorMenuButton::click);
    }

    @Override
    public void renderBg(@NotNull PoseStack mstack, @NotNull Minecraft mc, int mouseX, int mouseY) {
    ...
    public static class OpenConfigButtonHandler {
        ...
        public static void onGuiInit(ScreenEvent.Init event) {
            ...
            if (screen instanceof TitleScreen) {
                ...
                rowIdx =  AllConfigs.client().mainMenuConfigButtonRow.get() == 0 ? 4 : AllConfigs.client().mainMenuConfigButtonRow.get() - 1;
                offsetX = AllConfigs.client().mainMenuConfigButtonOffsetX.get();
            } else if (screen instanceof PauseScreen) {
                menu = MenuRows.INGAME_MENU;
                rowIdx = AllConfigs.client().ingameMenuConfigButtonRow.get() == 0 ? 5 : AllConfigs.client().ingameMenuConfigButtonRow.get() - 1;
            ...
            event.getListenersList()
                    ...
                    .findFirst()
                    .ifPresent(w -> toAdd
                            .setValue(new OpenDecorMenuButton(w.x + offsetX_ + (onLeft ? -20 : w.getWidth()), w.y)));
